...
{
    public class GraphSharePointService : GraphServiceBase
    {
        ...
        public GraphSharePointService(
            ...
            IOptions<TimeTrackerOptions> timeTrackerOptions) : base(authProvider, userContext)
        {
        ...
        public async Task<GraphResultItem> GetSiteListItemAsync(SiteList siteList, string itemId)
        {
            try
            {
                if (String.IsNullOrEmpty(siteList.ListId)) throw new ArgumentNullException(nameof(siteList.ListId));
                ...
                // Call to Graph API
                var graphResponse = await GraphClient.Sites[siteList.SiteId].Lists[siteList.ListId].Items[itemId].Request().GetAsync();
                ...
                foreach (var item in graphResponse.Fields.AdditionalData)
                {
                    resultsItemProperties.Add(item.Key, item.Value.ToString());
                }

                var resultsItem = new GraphResultItem
                {
                    Id = graphResponse.Id,
        ...
        public async Task<List<GraphResultItem>> GetSiteListItemsAsync(SiteList siteList)
        {
            try
            {
                ...
                var graphResponse = await GraphClient.Sites[siteList.SiteId].Lists[siteList.ListId].Items.Request().Expand("fields").GetAsync();
        ...
        public async Task<List<GraphResultItem>> GetSiteListItemsAsync(SiteList siteList, IEnumerable<QueryOption> queryOptions)
        {
            try
            {
                ...
                graphResponse = (ListItemsCollectionPage)await GraphClient.Sites[siteList.SiteId].Lists[siteList.ListId].Items.Request(options).Expand("fields").GetAsync();

                if (graphResponse == null) return new List<GraphResultItem>();
                ...
                if (listItemResults?.Count == 0) return resultItems;
        ...
        public async Task<IListCollectionPage<GraphResultItem>> GetSiteListItemsAsync(SiteList siteList, IEnumerable<QueryOption> queryOptions, int pageSize = 10)
        {
            try
            {
                ...
                // Call to graph API to retrieve List Items filtered by dateQuery (match pattern: yyyyMMdd) example: for full month 201802
                var graphRequest = await GraphClient.Sites[siteList.SiteId].Lists[siteList.ListId].Items.Request(options).Expand("fields").GetAsync();
        ...
        private async Task<bool> TryGetSiteListAsync(SiteList spSiteList)
        {
            try
            {
                ...
                graphRequest = await GraphClient.Sites[spSiteList.SiteId].Lists[spSiteList.ListId].Request().GetAsync();
