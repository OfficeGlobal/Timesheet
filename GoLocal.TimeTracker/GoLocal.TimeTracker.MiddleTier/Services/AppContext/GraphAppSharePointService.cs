...
namespace GoLocal.TimeTracker.MiddleTier.Services.AppContext
{
    public class GraphAppSharePointService : GraphAppServiceBase
    {
        ...
        public GraphAppSharePointService(
            ...
            ILogger<GraphAppSharePointService> logger,
        ...
        public async Task<List<GraphResultItem>> GetSiteListItemsAsync(SiteList siteList, IEnumerable<QueryOption> queryOptions)
        {
            try
            {
                ...
                graphResponse = (ListItemsCollectionPage)await GraphAppClient.Sites[siteList.SiteId].Lists[siteList.ListId].Items.Request(options).Expand("fields").GetAsync();
        ...
        public async Task<bool> TryGetSiteListAsync(SiteList spSiteList)
        {
            try
            {
                // Call to Graph API to check if SharePoint List exists.
